      ...
      <main className="mx-auto max-w-5xl px-4 py-8 sm:py-12">
        ...
        {overview && (
          <div className="mb-8 flex flex-wrap items-center gap-3">
            ...
            <Button variant="outline" size="sm" className="sm:ml-auto" onClick={() => void refresh()}>
              {t.admin.refresh}
            </Button>
          </div>
        )}
        ...
        {!loading && !forbidden && !error && rows.length === 0 && (
          <p className="py-8 text-text-secondary">{t.admin.empty}</p>
        )}

        {!loading && !forbidden && !error && rows.length > 0 && (
          <div className="overflow-x-auto rounded-card border border-border-card bg-white shadow-card-border">
                ...
                {rows.map((row) => (
                  <tr key={row.partner_id} className="border-b border-border-card last:border-b-0">
                    ...
                    <td className="px-4 py-3">
                      <PillBadge tone={TONE_BY_STATUS[row.status]}>{t.admin.statusLabels[row.status]}</PillBadge>
                    </td>
