...
> **APPLIQUÉ le 2026-09-16** (démonstrateur v2, lot L1b) : migration `supabase/migrations/20260915210000_transmissions_f1.sql` — la policy « Authenticated users can read with access_code » est supprimée et remplacée par `public.get_transmission_by_code(p_code text)` (`security definer`, `set search_path = ''`, `char_length(p_code) between 4 and 64`, `limit 1`, EXECUTE au seul rôle `authenticated`) ; la route `GET /api/transmission/:code` est rebranchée sur la RPC par le lot L2a. Contrat : `docs/design-v2-demonstrateur.md` §3.3.15. Le produit transmission reste gelé : **seul** ce correctif est appliqué, les 4 policies owner restent.
>
> **Résidu assumé** (revue adversariale L1/L1b, m7 ; contrat §9.3-10) : la RPC gardant `grant execute … to authenticated`, elle est appelable **en direct via PostgREST** avec la clé publishable, donc hors d'atteinte de tout limiteur Express — le devinage de code n'est pas étranglé côté base. C'est sans commune mesure avec le défaut corrigé (l'ancienne policy rendait **toute** la table lisible sans aucun code), et les codes sont des valeurs héritées de la v1 (plus aucun code n'est généré). Point de contrôle avant la bêta : `select count(*), min(char_length(access_code)), max(char_length(access_code)) from public.transmissions;` en prod — si des codes courts existent en nombre, relever la borne basse de longueur ou révoquer le grant et ne servir la RPC que par le serveur.
