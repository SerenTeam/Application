...
| `POST /api/partner/dossiers/:id/resend` | `requireAuth → killSwitch(PARTNER_ACTIVATIONS_ENABLED) → requireRpcSecret (500 si absent) → resendLimiter (20/h/user)` | `:id` UUID (sinon 404 sans appel base), `{ lang? }` | 200 `{ success, dossier, partner_name, email_sent, activation_url? }` | 404 `DOSSIER_NOT_FOUND` ; 409 `DOSSIER_NOT_INVITABLE` ; 429 `ROTATION_TOO_SOON` / `ROTATION_LIMIT` ; 403 `NOT_A_PARTNER` ; 500 |
