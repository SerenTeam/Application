# Checklist U1 — Déploiement du chantier 2a en préprod (mardi 15/09, ~19h15 → 20h45)

> Créneau 1 d'Arnaud du plan v2-démonstrateur. Objectif : la préprod tourne le code `preprod-2a-base`
> (2a envoi papier + dashboard PF v0 + socle L0) sur une base à 14/14 migrations, **avant** que les lots v2
> n'arrivent (créneau U2, mercredi 12h30). Aucune action sur la prod hormis une **lecture** (étape B).
>
...
> `preprod-2a-base` est un tag local posé par la session sur `integration/v2-demo` après commit de L0.
...
✅ Attendu : liste des versions + 3 compteurs + type d'instance, **collés dans la session** (sert à l'inventaire de la bêta prod).

---

## C. Git — pousser la base de déploiement et lancer la CI · ~5 min (+ attente CI)
...
✅ Attendu : un SHA (le tag existe). 🛑 STOP si `unknown revision` → la session n'a pas encore posé le tag.
...
✅ Attendu : un run CI (tsc + vitest + build) qui passe à `completed success` en ~5 min. On peut enchaîner D pendant l'attente, **mais pas F** (le push `pre-prod` attend la CI verte). 🛑 STOP si la CI échoue → coller le lien du run.
...
✅ Attendu : 14 lignes Local ; Remote renseigné **au moins** pour `20260701000000` → `20260716120000` (6 migrations) ; **aucune** version Remote absente de Local. 🛑 STOP sinon (ne JAMAIS lancer `migration repair` seul) → coller la sortie.
...
🟠 Plan B si erreur **42501 « must be owner of table objects »** (policies Storage) : Dashboard préprod → Storage → bucket `documents` → Policies : créer les 3 policies décrites dans `supabase/migrations/20260914160000_attachments.sql` (section policies `storage.objects`), puis relancer `supabase db push`.
...
## E. Secret RPC en base PRÉPROD · ~3 min
...
✅ Attendu : exactement 1 ligne `id = 1`. La même valeur ira dans Render (étape F).
...
**À laisser absentes / retirer** : `FEATURE_LLM`, `EMAIL_SENDS_ENABLED`, `EXTRA_SENDS_ENABLED`, `PAYMENTS_ENABLED`, `STRIPE_PRICE_ID` (forfait famille abandonné en v2).

Puis, **CI de l'étape C verte** et **étape D terminée** :
...
| Critère | OK ? |
|---|---|
| CI verte sur `integration/v2-demo` | ☐ |
...
| Inventaire prod (étape B) collé dans la session | ☐ |
| Deploy ID préprod noté | ☐ |

**NO-GO** (un critère rouge non corrigeable dans le créneau) : ne pas insister — `pre-prod` reste sur son ancien commit si F n'a pas eu lieu ; sinon Render → Rollback vers le déploiement précédent. Le créneau U2 poussera alors 2a + v2 ensemble (la session recalcule la liste attendue du dry-run).
