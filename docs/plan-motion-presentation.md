...
- **Task 10 (revue qualité)** : deux durcissements appliqués (code du Step 1 corrigé ci-dessous) — garde `if (e.repeat) return;` (touche maintenue sur clavier de kiosque = rafale de keydown) et `.catch(() => {})` sur les deux promesses fullscreen (contexte embed → rejection silencieuse, protège le critère « zéro erreur console »). API fullscreen non préfixée assumée (navigateur de lecture moderne).
- **Task 9 (revue qualité)** : approuvée sans correctif — raccord de boucle prouvé sans flash (sonde à 240 fps sur 2 itérations : dernier frame blanc = premier frame blanc, 0 frame de fuite bleue après 30,0), seek arrière robuste (l'overlay se rembobine à 0), resets S5 « stale-but-hidden » sains. Deux nits repliés en T10 (même fichier) : commentaire croisé sur le reset de l'overlay (L148 → « reset à 0 par scene1 ») et formulation du commentaire de l'assert (« ≈ 30 s »).
...
function scene1() {
  ...
  tl.set("#s1", { visibility: "hidden" }, 5.76); // après la fin de la convergence : fondu naturel sous la carte S2
  return tl;
}
...
addEventListener("keydown", e => {
  if (e.repeat) return; // touche maintenue (kiosque) : un seul déclenchement
