...
- [x] **1.1** Écrire la migration (complète, style du repo — commentaires FR) :
...
- [x] **1.2** Vérifier : `npx tsc --noEmit` (0 err) ; `npx vitest run` (199/199 — non-régression) ; relecture SQL (rejouable base vierge ET préprod).
- [x] **1.3** Commit : `git add supabase/migrations/20260914100000_sender_profiles_organisations.sql && git commit -m "feat(2a): tables sender_profiles + organisations (RLS, lignes d'adresse ≤45)"`
...
- [x] **3.1** TDD : test invariants d'abord — ajouter `deceased_department` à `IDENTITY_FIELDS` (`tests/invariants.test.ts:14`) et un test « la question departement existe, type select, 101+ options, jamais transmise au rédacteur » (vérifier contre `writerContext()` dans `server/routes/questionnaire.js:35-42` : le champ NE doit PAS y apparaître — test négatif).
- [x] **3.2** Implémenter : question `deceased_department` (type `select`, bloc identité, options depuis `server/lib/departments.js`, textes `{fr,en}`) ; champ optionnel dans `QuestionnaireAnswersV2` (`deceased_department?: string`) ; les vieux dossiers sans département : le panneau d'envoi le demandera (Task 9), aucune migration de données.
- [x] **3.3** `npx vitest run` → suite verte (nouveaux tests inclus) ; `npx tsc --noEmit`. Commit : `feat(2a): question département du défunt — donnée d'adressage, exclue du rédacteur (invariants à jour)`
...
- [x] **4.1** Migration (les blocs, dans cet ordre) :
...
- [x] **4.2** Relecture SQL complète (matrice de transitions écrite en commentaire dans la migration) ; `npx tsc --noEmit`. Commit : `feat(2a): migration cycle papier — statuts, débits atomiques, resend_of, provider_events, plafonds, RLS lecture seule (⚠️ store rebranché en Task 5)`
...
- [x] **6.1** TDD routes (mocks storage) → **6.2** implémentation (upload au client `getSupabaseClient(token)` — la RLS storage s'applique ; URLs signées 300 s générées serveur, jamais stockées) → **6.3** suite verte + tsc → **6.4** commit `feat(2a): coffre minimal — bucket privé, magic bytes, RLS par préfixe`
...
- [x] **8.1** TDD parité + rendu (variables manquantes → erreur listée, jamais de `{{...}}` résiduel dans un corps rendu — test). **8.2** Implémentation. **8.3** Suite verte. **8.4** Commit `feat(2a): templates jumeaux serveur + regénération du corps (fin du corps libre client)`
...
- [x] **9.1** TDD la matrice de gardes (ordre exact ci-dessus) → **9.2** implémentation → **9.3** suite verte + boot check (`PORT=3999`, POST sans kill switch → 503) → **9.4** commit `feat(2a): route envoi papier — gate, quota atomique, facturation à l'acte, kill switch, plafonds`

> **Note post-revue (Task 9, 2026-09-13 — 2 passes, APPROUVÉE) :** revue adversariale → 1 critique (cul-de-sac du dedup_key : quota épuisé → achat d'un envoi sup → 409 définitif) + 2 importants, tous fermés à la racine : **reprise branchée** (`claim_letter_retry`, garde 7 bis — même ligne → même Idempotency-Key, structurellement pas de second pli ; la reprise revalide TOUTE la chaîne de gardes dans la requête courante) ; **3 régimes d'échec provider** (4xx/5xx-avec-status → libéré+failed ; rejet réseau = résultat INCERTAIN → débit conservé, ligne `prepared`, 503 retryable — un pli peut-être parti n'est jamais abandonné) ; `normalizeKind` refuse au lieu de deviner. Déviations actées : kill switch remonté en middleware AVANT le limiteur (un 503 de coupure ne consomme pas le quota horaire) ; sonde M1 déplacée du webhook (client anon : lecture purchases impossible) vers `GET /payments/status` (validé par le reviewer, suggestion initiale retirée). **Legs** : R1 → Task 11 : porter `retry_after_seconds` dans le 402 et tolérer un `SEND_IN_PROGRESS` unique post-achat (fenêtre de 120 s) — ne JAMAIS marquer `failed` pour contourner (ça fausserait les plafonds) ; R2 → Task 11/2b : refuser la reprise dont l'ensemble de PJ diffère du persisté (traçabilité juridique de « ce qui a été posté ») ; R3 (limite connue) : les lignes du régime incertain échappent au plafond global le temps de la resync — le limiteur 20/h/user reste la vraie borne.
...
- [x] **10.1** TDD → **10.2** implémentation (timer resync : envois `submitted` > 24 h, `sent` ≤ J+30, **et `prepared` avec `provider_ref` non nul** — cas du serveur mort entre le POST provider et l'enregistrement du résultat, revue Task 4 — via `getLetter` + fold + RPC) → **10.3** suite verte, boot check → **10.4** commit `feat(2a): webhook-ping MSB (persist+verify-by-GET) + resynchronisation périodique serveur`
...
- [x] **11.1** Requalification canaux + test → **11.2** panneau (TDD là où testable, sinon build+tsc) → **11.3** suite verte + build → **11.4** commit `feat(2a): panneau envoi papier — profil, adresse, PJ, quota, à-l'acte, wording sent-final (FR/EN)`

> **Note post-revue (Task 11, 2026-09-14) :** legs R1/R2 de la revue Task 9 fermés dans CE lot (comme prévu) : **R1** — 402 `QUOTA_EXHAUSTED` porte désormais `retry_after_seconds: 120` (`server/routes/letters.js`, constante `PAPER_STALE_SECONDS` déjà existante réutilisée) ; **R2** — sur une reprise (branche duplicate → claim), un ensemble de `attachment_ids` différent de celui persisté sur la ligne d'origine refuse en 409 `ATTACHMENTS_MISMATCH` (comparaison en ENSEMBLE, ordre insensible), AVANT tout claim. Deux tests dédiés + non-régression (455/455, base 451 + 4).
>
> **Structure de composants retenue** (fichier `LetterSendPanel.tsx` gardé comme simple dispatcher paywall commun → email historique inchangé / `PaperSendPanel.tsx` nouveau) : `PaperSendPanel.tsx` (orchestrateur : profil, snapshot de reprise, quota, statuts, reprise post-Checkout) + 4 sous-composants extraits comme suggéré par le plan — `SenderProfileForm.tsx` (lecture/écriture directe `sender_profiles` par le client Supabase, RLS owner), `RecipientAddressForm.tsx` (annuaire réseau + repli saisie département + adresse toujours éditable), `AttachmentPicker.tsx` (coffre + upload inline + gel PJ), `QuotaBadge.tsx` (solde, purement présentationnel).
>
> **Écarts actés** (aucun ne dévie de la spec, tous nécessaires à un panneau fonctionnel) :
> 1. **Prix de l'envoi à l'acte** : `GET /api/payments/status` gagne un champ `extra_price` (même mécanique que `price`, lecteur Stripe dédié `getExtraPrice` câblé dans `server.js`) — sans lui, le bouton « Acheter un envoi (prix affiché) » explicitement demandé n'avait aucune source de prix. `usePayments()` expose `extraPrice` + `startExtraSendCheckout`. Tests ajoutés dans `tests/payments-routes.test.ts`.
> 2. **Département de secours (dossier ancien sans réponse à la question Task 3)** : saisie libre validée par regex (`^(2A|2B|[0-9]{2,3})$`, miroir du CHECK SQL `organisations.department`) plutôt qu'un catalogue de 101 libellés dupliqué côté front — la donnée n'a besoin que d'interroger `GET /api/letters/organisations`, jamais affichée en toutes lettres ailleurs. Persistée par `DashboardPage` (seul endroit qui détient les réponses ENTIÈRES du questionnaire, pour ne jamais écraser le reste du JSON `answers`) via un callback remonté par toute la chaîne de props.
> 3. **Reprise automatique post-Checkout (legs R1 UI)** : `src/lib/paper-send-resume.ts` persiste en `sessionStorage` (jamais `localStorage`) le payload EXACT refusé en 402 avant la redirection Stripe, et le rejoue UNE fois au retour (`?checkout=extra_success`, distinct de `checkout=success` du forfait). Parce que la section courrier est repliée par défaut, `StepItem`/`StepLetterSection` (RoadmapView) et `activeView` (DashboardPage) s'auto-ouvrent au premier rendu via une lecture non destructive (`shouldAutoResumePaperSend`/`hasPendingPaperSendForCheckoutReturn`) — sans quoi `PaperSendPanel` ne remonterait jamais pour consommer la reprise.
> 4. **Gel des PJ étendu au-delà de la seule reprise post-Checkout** : toute réponse impliquant qu'une ligne existe désormais côté serveur (`QUOTA_EXHAUSTED`, `PAPER_NOT_CONFIGURED`, 503 `retryable`, échec certain `SEND_FAILED`/`PROVIDER_UNAVAILABLE`) fige `attachment_ids` sur ce qui vient d'être envoyé (fonction `freezeRow`) — sans ça, un « Réessayer » cliqué manuellement dans la même session (sans passer par Checkout) après un premier échec aurait pu renvoyer une sélection différente et se heurter au 409 R2 pour rien.
>
> **Tableau codes d'erreur → comportement UI** (voir rapport de tâche pour le détail).

> **Note post-revue (Tasks 10+11, revue finale combinée, 2026-09-14 — APPROUVÉES après correctifs) :** 2 critiques fermés — C1 : le webhook ne peut plus être coincé par un `send_id` non-uuid/inconnu (validation + retente sans send_id ; le test qui encodait le bug corrigé) ; C2 : plus de double paiement possible au retour de Checkout (poll `status='paid'` borné avant rejeu, bandeau « confirmation » au lieu de l'offre d'achat). Importants fermés : idempotence webhook « déjà traité » (relivraison d'un non-traité → retraitement) ; resync réorientée vers les VRAIS orphelins (`prepared` débités sans ref, > 2 h → Sentry `orphan_debited_send` — réconciliation provider = 2b) ; bouton d'achat piloté par le serveur seul ; adresse figée sur reprise (elle entre dans le dedup_key — sinon double débit) ; retry auto strictement unique. Polish : ids DOM par `useId`, gel élargi, doublons de PJ refusés (400), nettoyage sessionStorage inconditionnel, parité `NETWORK_RECIPIENT_TEMPLATES` testée. **LOT 2a CODE-COMPLET : 467 tests / 27 fichiers (base 199 au début du chantier).** Reste : Task 12 (post-gel) + user steps E2E.
