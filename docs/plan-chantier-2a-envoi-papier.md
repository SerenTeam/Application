...
| `letter.returned_to_sender` | `failed_address` si `wrong_address: true`, sinon `failed` | sémantique à confirmer sur payloads réels |
| `letter.error` | `failed` | |
...
- [ ] **5.1** TDD : adapter les tests du store aux nouveaux appels RPC (mock `client.rpc(...)` au lieu de `from().insert()`) — écrire d'abord, voir échouer.
...
- [ ] **10.1** TDD → **10.2** implémentation (timer resync : envois `submitted` > 24 h et `sent` ≤ J+30, via `getLetter` + fold + RPC) → **10.3** suite verte, boot check → **10.4** commit `feat(2a): webhook-ping MSB (persist+verify-by-GET) + resynchronisation périodique serveur`
