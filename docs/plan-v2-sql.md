...
- **E7 — Copie `attributions` → `dossiers` non défensive.** Si un compte de démo de la préprod a une adresse qui viole `dossiers_email_check` (format, > 254), l'INSERT de la migration échoue et **tout le push U2 échoue**. Le SQL du contrat n'est pas modifié ; à ajouter à la checklist U2, avant le `db push`, en lecture sur la préprod : `select count(*) from public.attributions a join auth.users u on u.id = a.user_id where lower(btrim(u.email)) !~ '^[^@\s]+@[^@\s]+\.[^@\s]+$' or char_length(u.email) > 254;` → doit valoir 0.
  **Sonde complétée le 16/09 (revue L1/L1b, m6)** : deux autres cas font échouer le push exactement de la même façon et doivent entrer dans la même vérification (la requête ci-dessus, avec sa longueur > 254, couvre déjà le cas (b) ; le cas (a) est neuf) — (a) **collision d'adresses après normalisation** : deux comptes `attributions` dont les e-mails ne diffèrent que par la casse ou des espaces violeraient `dossiers_family_email_open_uidx` → `select lower(btrim(u.email)) as e, count(*) from public.attributions a join auth.users u on u.id = a.user_id group by 1 having count(*) > 1;` → **doit renvoyer 0 ligne** ; (b) adresse > 254 caractères, déjà dans la requête ci-dessus. 🛑 STOP sur tout résultat non vide : corriger ou exclure le compte de démo **avant** le `db push`.
...
Correctifs de la **revue adversariale du lot L1/L1b** (16/09, correcteur L1/L1b) :

- [2026-09-16] [Task 3] **défaut critique C1 — statut du partenaire au renvoi et à l'annulation**. `partner_rotate_invitation` résolvait le partenaire sans aucun filtre de statut et ne levait jamais `partner_inactive` ; `partner_cancel_dossier` ne joignait même pas `partners`. Une PF **suspendue** ou **résiliée** conservait donc, sur chaque dossier `invited` dont elle connaissait l'id : la relecture de la PII famille et défunt renvoyée par la RPC, l'invalidation du lien d'activation en attente, et le déclenchement d'un e-mail à la famille en son nom — alors que `my_account()` renvoie déjà `role='none'` pour un contrat résilié (la révocation n'existait qu'à l'écran). Cause : contradiction interne du contrat, l'« ordre strict » des §3.3.10/§3.3.11 ne comportant aucun contrôle de statut là où le §3.3.6 énonce que `'suspended'` bloque « la création **et le renvoi** » et que `'terminated'` retire tout rôle PF. **Correctifs** : (1) renvoi — `p.status` lu à l'étape 1, étape 1bis `coalesce(v_status,'') <> 'active'` → `partner_inactive` ; (2) annulation — résolution du partenaire avec `and p.status <> 'terminated'` → `not_a_partner`. **Contrat amendé** : §3.3.10 étape 1bis, §3.3.11 étape 1, §4.4 (ligne *resend* : 403 `PARTNER_INACTIVE`), notes §12.2. Aucun changement serveur ni front (`partner_inactive` et `not_a_partner` sont déjà mappés). Scénarios neufs S3x-S3x4 et S3y-S3y3, et règle de lint « renvoi et annulation contrôlent le statut du partenaire ». (2) est une **décision produit appliquée par défaut, à confirmer par Arnaud** (réversible : retirer la clause et le scénario S3y2). — correcteur L1/L1b.
- [2026-09-16] [Task 3] m2, m3, m4 — trois gardes à trois états et une normalisation asymétrique : `rotation_too_soon` comparait `v_d.invite_issued_at` sans `coalesce` (or `dossiers_state_check` n'exige pas cette colonne pour un dossier `'invited'` : une ligne posée par un seed ou un backfill rendait la comparaison NULL, donc fausse, et l'étranglement contournable) → `coalesce(v_d.invite_issued_at, v_d.created_at)` ; `partner_create_dossier` étape 2 testait `v_partner.status <> 'active'`, qui ne lève rien si le statut est NULL → `coalesce(v_partner.status, '')` ; étape 5, `lower(u.email)` devient `lower(btrim(u.email))`, la normalisation utilisée partout ailleurs. Aucun scénario existant modifié. — correcteur L1/L1b.
- [2026-09-16] [Task 7] m7 — résidu du correctif F1 documenté, pas corrigé : `get_transmission_by_code` garde `grant execute … to authenticated`, donc reste appelable en direct via PostgREST, hors d'atteinte du limiteur Express — le devinage de code n'est pas étranglé côté base. Écrit au contrat §9.3-10 (avec le point de contrôle prod sur la longueur réelle des codes) et dans `docs/audit-rls.md` F1, dont la mention « Application différée » devient « APPLIQUÉ le 2026-09-16 ». — correcteur L1/L1b.
- [2026-09-16] [Task 2] m1 — **piège d'exploitation à tenir en U2, non corrigé** : `account_enrollments_email_role_uidx` autorise une même adresse en `partner_manager` **et** `seren_admin`, mais `link_enrollments` teste l'idempotence sur l'ensemble des enrôlements de l'adresse (§3.3.3 étape 2, suivi à la lettre). Si le premier rôle est déjà lié à un compte qui s'est depuis connecté, le second enrôlement rejoue les contrôles de confiance sur ce compte et échoue en `enrollment_account_untrusted`. Conséquence pratique : **ne jamais enrôler la même adresse sous deux rôles** ; si le cas se présente, utiliser deux adresses. Corriger le test d'idempotence pour le restreindre à la paire courante serait une déviation du §3.3.3 et toucherait une fonction couverte par S14a-m : écarté à chaud, à rouvrir après la bêta. — correcteur L1/L1b.
- [2026-09-16] [Tasks 2, 3] m5 — **ces deux migrations ne sont pas un véhicule d'amendement** : `create unique index if not exists` et les `add constraint` en bloc `do … exception when duplicate_object` conservent silencieusement l'objet existant si son **prédicat** diffère. Sans effet en U2/U4 (premier push, objets absents), mais tout correctif ultérieur d'une contrainte ou d'un index devra passer par une **migration neuve avec drop explicite**, jamais par la retouche de `20260915200000` / `20260915201000`. — correcteur L1/L1b.
- [2026-09-16] [Task 5, Task 0] m10 — **effet de bord sur la stack locale partagée** : le bloc `[auth.hook.before_user_created]` de `supabase/config.toml` est actif pour **tous** les lots depuis la Task 5, et n'est relu qu'au `supabase stop` / `supabase start` — un lot qui enchaîne un `db reset` sans redémarrage peut croire le hook inactif (ou, à l'inverse, se faire refuser une inscription de test). Toute création de compte par `signUp` dans un harnais local doit donc soit passer par un jeton d'invitation valide, soit être précédée d'un enrôlement. — correcteur L1/L1b.
- [2026-09-16] [Task 0] m8 — harnais SQL : les 5 scripts (`v2-env.sh`, `psql-local`, `with-db-lock`, `hosted-grants.sql`, `run-sql-checks`, `gate`) vivent dans le scratchpad, mais leur **source complète est versionnée** dans ce plan (Task 0 Step 2) : le harnais est reproductible depuis le dépôt seul, aucune preuve n'est perdue avec le scratchpad. Ajout d'un rappel en tête de `scripts/sql-scenarios-v2.sql` : sans `hosted-grants.sql` rejoué après chaque `db reset`, S12 (deny-all) est un **faux vert**. Aucun fichier dupliqué sous `scripts/`. — correcteur L1/L1b.
- [2026-09-16] [Task 2] m9 — **non corrigé, assumé** : l'étape 9 de `claim_dossier` (`account_already_linked`) est un `exists` non verrouillé ; deux claims simultanés du **même compte** sur **deux jetons valides distincts** passent tous deux l'étape et le second échoue en `unique_violation` brute (23505) sur `dossiers_user_uidx` → 500 `ACTIVATION_ERROR` au lieu d'un code métier. L'invariant tient (jamais deux dossiers pour un compte), seul le message est laid, et le scénario (deux invitations valides pour la même personne, activées à la même seconde) est trop étroit pour justifier un verrou supplémentaire avant la démo. — correcteur L1/L1b.
