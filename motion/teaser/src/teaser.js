"use strict";
// Teaser Seren — 1 lecture (pas de boucle), coupes franches (wipe), FR uniquement.
...
// ---------- helper : coupe franche (wipe) ----------
const WIPE_IN = 0.16, WIPE_OUT = 0.18;
function cut(tl, at) {
  tl.set("#wipe", { scaleX: 0, transformOrigin: "0% 50%" }, at);
  tl.to("#wipe", { scaleX: 1, duration: WIPE_IN, ease: "power4.in" }, at);
  tl.set("#wipe", { transformOrigin: "100% 50%" }, at + WIPE_IN);
  tl.to("#wipe", { scaleX: 0, duration: WIPE_OUT, ease: "power4.out" }, at + WIPE_IN);
  return { cover: at + WIPE_IN, next: at + WIPE_IN + WIPE_OUT };
}
...
tl.to("#t1 .tt", { scale: 1.035, duration: 0.2, ease: "power2.in" }, T1_START + T1_DUR - 0.2);

let c = cut(tl, T1_START + T1_DUR);
tl.set("#t1", { visibility: "hidden" }, c.cover);

// ================= V1 : débordement de papiers =================
const V1_START = c.next, V1_DUR = 2.5;
const ROT = [-9, 7, -13, 6, -6, 11, -8, 5, -12, 8];
tl.set("#v1", { visibility: "visible" }, c.cover);
...
tl.to("#v1 .paper", { scale: 1.06, opacity: 0, duration: 0.3, ease: "power2.in", stagger: 0.01 }, V1_START + V1_DUR - 0.3);

c = cut(tl, V1_START + V1_DUR);
tl.set("#v1", { visibility: "hidden" }, c.cover);

// ================= T2 : "Quelques questions." =================
const T2_START = c.next, T2_DUR = 1.1;
tl.set("#t2", { visibility: "visible" }, c.cover);
...
tl.to("#t2 .tt", { scale: 1.035, duration: 0.18, ease: "power2.in" }, T2_START + T2_DUR - 0.18);

c = cut(tl, T2_START + T2_DUR);
tl.set("#t2", { visibility: "hidden" }, c.cover);

// ================= V2 : questionnaire =================
const V2_START = c.next, V2_DUR = 2.9;
tl.set("#v2", { visibility: "visible" }, c.cover);
...
tl.to("#v2-card", { scale: 1.03, duration: 0.28, ease: "power2.in" }, V2_START + V2_DUR - 0.28);

c = cut(tl, V2_START + V2_DUR);
tl.set("#v2", { visibility: "hidden" }, c.cover);

// ================= T3 : "Votre parcours, dans le bon ordre." =================
const T3_START = c.next, T3_DUR = 1.3;
tl.set("#t3", { visibility: "visible" }, c.cover);
...
tl.to("#t3 .tt", { scale: 1.03, duration: 0.18, ease: "power2.in" }, T3_START + T3_DUR - 0.18);

c = cut(tl, T3_START + T3_DUR);
tl.set("#t3", { visibility: "hidden" }, c.cover);

// ================= V3 : roadmap =================
const V3_START = c.next, V3_DUR = 2.9;
tl.set("#v3", { visibility: "visible" }, c.cover);
...
tl.to("#v3-card", { scale: 1.025, duration: 0.28, ease: "power2.in" }, V3_START + V3_DUR - 0.28);
...
tl.to("#t4 .tt", { scale: 1.035, duration: 0.18, ease: "power2.in" }, T4_START + T4_DUR - 0.18);
...
tl.set("#sig", { visibility: "visible" }, c.cover);
