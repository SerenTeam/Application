...
addEventListener("keydown", e => {
  if (e.repeat) return; // touche maintenue (auto-repeat) : on n'agit qu'à l'appui initial
  if (e.code === "Space") { e.preventDefault(); tl.paused(!tl.paused()); }
  else if (e.key === "f" || e.key === "F") {
    // promesses couvertes : en contexte embed/edge le refus devient silencieux (pas d'erreur console)
