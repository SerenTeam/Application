...
- **Tests** : Vitest (`npm test`) — exécutés en CI (`.github/workflows/ci.yml`) sur chaque push/PR — moteur, catalogues, invariants croisés, routes (supertest). **199 sur `main`, 467 sur `feature/chantier-2a`.** Les invariants interdisent toute question sans étape et tout drift entre catalogues. **Sans `.env`** (session cloud, CI) : exporter les 2 variables factices `VITE_SUPABASE_URL=http://localhost:54321` et `VITE_SUPABASE_PUBLISHABLE_KEY=sb_publishable_ci_dummy` (sinon 2 fichiers échouent à l'import — aucun test ne fait de réseau, cf. `ci.yml`)
